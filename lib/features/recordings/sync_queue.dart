...
class SyncQueue {
  ...
  Future<int> flush() async {
    ...
    var uploaded = 0;
    for (final item in await database.pendingRecordings()) {
      try {
        await database.updateStatus(item.id, 'uploading');
        ...
        await database.updateStatus(item.id, 'uploaded');
        uploaded++;
      } catch (error) {
        await database.updateStatus(item.id, 'failed', error: error.toString());
      }
    }
    return uploaded;
  }
